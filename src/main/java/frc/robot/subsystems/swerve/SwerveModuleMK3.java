...
public class SwerveModuleMK3 {
  ...
  private final RobotSpecs robotSpecs;
  ...
  public SwerveModuleMK3(RobotSpecs robotSpcs, CANSparkMax driveMtr, CANSparkMax angleMtr, double offsetDegrees, CANCoder absEnc,
      boolean invertAngleMtr, boolean invertAngleCmd, boolean invertDrive, String prefix) {
    robotSpecs = robotSpcs;
    driveMotor = driveMtr;
    angleMotor = angleMtr;
    absEncoder = absEnc;
    myprefix = prefix;
    ...
    driveEncoder.setPositionConversionFactor(Math.PI * robotSpecs.chassisConfig.wheelDiameter / robotSpecs.chassisConfig.kDriveGR); // mo-rot to wheel units
    driveEncoder.setVelocityConversionFactor((Math.PI * robotSpecs.chassisConfig.wheelDiameter / robotSpecs.chassisConfig.kDriveGR) / 60.0); // mo-rpm wheel units 
    sleep(100);
    // Angle Motor config
    ...
    angleEncoder.setPositionConversionFactor(360.0 / robotSpecs.chassisConfig.kSteeringGR); // mo-rotations to degrees
    angleEncoder.setVelocityConversionFactor(360.0 / robotSpecs.chassisConfig.kSteeringGR / 60.0); // rpm to deg/s
  ...
  void realityCheckSparkMax(double angle_cancoder, double internal_angle) {
    ...
    if (Math.abs(
        driveEncoder.getPositionConversionFactor() - Math.PI * robotSpecs.chassisConfig.wheelDiameter / robotSpecs.chassisConfig.kDriveGR) > 0.1) {
      System.out.println("*** ERROR *** " + myprefix + " position conversion factor incorrect for drive");
      System.out.println("Expected Position CF: " + Math.PI * robotSpecs.chassisConfig.wheelDiameter / robotSpecs.chassisConfig.kDriveGR);
    ...
    if (Math.abs(driveEncoder.getVelocityConversionFactor()
        - Math.PI * robotSpecs.chassisConfig.wheelDiameter / robotSpecs.chassisConfig.kDriveGR / 60.0) > 0.1) {
      System.out.println("*** ERROR *** " + myprefix + " velocity conversion factor incorrect for drive");
      System.out.println("Expected Vel CF: " + Math.PI * robotSpecs.chassisConfig.wheelDiameter / robotSpecs.chassisConfig.kDriveGR / 60.0);
      System.out.println("Returned Vel CF: " + driveEncoder.getVelocityConversionFactor());
      result = false;
    }
    if (Math.abs(angleEncoder.getPositionConversionFactor() - (360.0 / robotSpecs.chassisConfig.kSteeringGR)) > 0.1) {
      System.out.println("*** ERROR *** " + myprefix + " position conversion factor incorrect for angle");
      System.out.println("Expected Angle Pos CF: " + 360.0 / robotSpecs.chassisConfig.kSteeringGR);
      System.out.println("Returned Angle Pos CF: " + angleEncoder.getPositionConversionFactor());
      result = false;
    }
    if (Math.abs(angleEncoder.getVelocityConversionFactor() - (360.0 / robotSpecs.chassisConfig.kSteeringGR / 60)) > 0.1) {
      System.out.println("*** ERROR *** " + myprefix + " velocity conversion factor incorrect for angle");
      System.out.println("Expected Angle Vel CF: " + (360.0 / robotSpecs.chassisConfig.kSteeringGR / 60));
